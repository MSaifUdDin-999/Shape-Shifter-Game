...
export default function ShapeShifterGame() {
  ...
  const spawnIntervalRef = useRef(null);  // Controls shape creation timer.
  const gameTickRef = useRef(null);       // Controls game tick timer for moving shapes and collision detection.
  const areaRef = useRef(null);           // Reference to the game area div for size calculations.
  const controls = useAnimation();

  // Spawn logic
  useEffect(() => {
    function spawnShape() {   // Create a new falling shape with random type, position, and speed
      const shapeType = SHAPE_KEYS[randomInt(0, SHAPE_KEYS.length - 1)];  // Randomly select a shape
      const left = randomInt(8, 92);        // Random horizontal position.
      const speed = randomInt(2000, 5000);  // Random speed - ms to fall
      const id = Math.random().toString(36).slice(2, 9);  // Unique ID for the shape
      setShapes((s) => [  // Add the new shape to the list of falling shape
        ...s,
        {
          id,
          type: shapeType,
    ...
    if (running) { // Start spawning shapes at intervals when the game is running
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-indigo-900 to-slate-800 text-white p-6 flex items-center justify-center">
      <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 text-center text-sm text-gray-300">
          Coded with ❤️ By <b>"<a className="color-white hover:underline" href="https://portfolio-m-saif-ud-din.vercel.app/" target="_blank" rel="noopener noreferrer">M Saif Ud Din</a>"</b>
        </div>
      </div>
